...
class Alumno:
    def __init__(self,ventana):
        ...
        #Boton
        self.crear = ttk.Button(marco,text = "Guardar alumno", command = self.agregarRegistro)
        ...
        '''
        self.borrar = ttk.Button(marco,text = "Borrar alumno", command = self.borrarRegistro)
    ...
    def agregarRegistro(self):
        if len(self.nombre.get())!=0 and len(self.clave.get())!=0:
            ...
            self.mensaje['text'] = "El alumno "+self.nombre.get()+" se a insertado existosamente"
            self.nombre.delete(0,END)
            self.clave.delete(0,END)
            self.nombre.focus()
        else:
            self.mensaje['text'] = "El nombre y la clave del alumno, no pueden estar vacias"
        self.mostrarDatos()
    def editarRegistro(self):
        if len(self.nombre.get())!=0 and len(self.clave.get())!=0:
            query = "UPDATE alumnos set nombre = '"+self.nombre.get()+"', clave = '"+self.clave.get()+"' where clave = '"+self.claveVieja+"';"
            self.queryAlumnos(query)
            self.mensaje['text'] = "El alumno "+self.nombre.get()+" se a actualizado existosamente"
            self.nombre.delete(0,END)
            self.clave.delete(0,END)
            self.nombre.focus()
        else:
